 ...
 **/ 
public class DemoLowFrequency extends JFrame {
	private static final long serialVersionUID = 1L;
	private final Drawer drawer;

	
	
	public DemoLowFrequency(final BufferedImage[] images) {
...
//			File file = new File("images/test/bird.png");
		...
		long[][] longLevel1Array = ravelet.getHigherLevelLattice(level+1); 
		LongLattice levelLattice = new LongLattice(longLevel1Array, ravelet.ox, ravelet.oy, ravelet.min, ravelet.factor);

		HexagonalDisplay.render(levelLattice.getDequantizedLattice().getData(), dst);
		
        images[1] = new BufferedImage(sizex, sizey, BufferedImage.TYPE_BYTE_GRAY);
        ImageUtils.doubleArrayToImageRaster(images[1], dst);

		run(new DemoLowFrequency(images), sizex + 300, sizey + 100);
	}
